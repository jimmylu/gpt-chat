...
impl ChatFile {
    pub fn new(filename: String, data: &[u8]) -> Self {
    ...
    pub fn url(&self, ws_url: &str) -> String {
        format!("/files/{}/{}", ws_url, self.hash_to_path())
    }
...
mod tests {
    ...
    fn test_hash_to_path() {
        let file = ChatFile::new("test.txt".to_string(), b"hello");
        assert_eq!(
            file.hash_to_path(),
